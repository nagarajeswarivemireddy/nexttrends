...
class AllProductsSection extends Component {
  ...
  getProducts = async () => {
    ...
    const apiUrl =
      'https://apis.ccbp.in/products?sort_by={activeOptionId}&category={categoryId}&title_search={searchInput}&rating={ratingId}'
    const options = {
      headers: {
  ...
  failureView = () => (
    <div className="failureContainer">
      <img
        src="https://assets.ccbp.in/frontend/content/react-js/nxt-trendz-products-filter-group-lg-failure-view.png"
        alt="failure view"
      />
          ...
          <ul className="ul-list">
            {ratingsList.map(each => (
              <Rating
                key={each.id}
                ratingDetails={each}
                clickOnRating={this.clickOnRating}
              />
            ))}
          </ul>
        </div>
